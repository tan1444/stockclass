...
export = {
    ...
    createTradeNo() {
        let tradeNo = moment().format("YYYYMMDDHHmmSSSS");
        return tradeNo;
    },
    getClientIp(req, proxyType) {
        let ip = req.socket.remoteAddress || (req.connection.socket ? req.connection.socket.remoteAddress : null);
        // 如果使用了nginx代理
        if (proxyType === 'nginx') {
